	...
	<view>
		...
		<view>
			...
			<view class="content">
				<view class="detail">
					<view class="d-list">
						<view v-for="(item2,index) in materModelList" :key="index">
							<view class="materials">
								<image class="img" :src="item2.mat_img?item2.mat_img:imgSrc"></image>
								<view class="name">
									<view class="n-va">{{item2.mat_title}}</view>
									<view class="n-va">规格: {{item2.model}}</view>
								</view>
								<view class="remove">
									<uni-number-box class="n-btn" :min="0" :value="item2.addSum" 
										@change="matNumberChange($event,index,true)"></uni-number-box>
									<view class="btn" :style="{
										background: item2.select==1?'#1296db':'',
										color:item2.select==1?'#f1f1f1':''}" 
										@click="removeMaterials(index)">
										{{item2.select==1?'已选择':'选择'}}</view>
								</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			//明细数量
			matNumberChange(e, index,needChange) {
				if(needChange){
					if(_self.materModelList[index].addSum == 0 && e > 0)
					{
						_self.removeMaterials(index)
					}
					if(_self.materModelList[index].addSum > 0 && e == 0)
					{
						_self.removeMaterials(index)
			...
			removeMaterials(index) {
				if(_self.materModelList[index].select==1)
				{
					_self.matNumberChange(0,index,false)
				}else{
					_self.matNumberChange(1,index,false)
				}
